...
class Test_goodkind(select1):
    ...
    def test_add_goodkind(self):
        ...
        with allure.step('输入分类名称'):
            ...
            self.input('//*[@class="z-form-limit"]/div[1]/input', '自动化测试1')
        with allure.step('点击确认按钮'):
            self.click('//*[@aria-label="新建分类"]/div[3]/div/div/div[2]/button[2]/span')
        with allure.step('审查是否出现新的分类'):
            time.sleep(1)
            assert self.getText(
                '//*[@id="app"]/div/div[2]/section/div/div/div[4]/div[1]/div/ul/li[5]/div[1]') == '自动化测试1 (0)'
    ...
    @pytest.mark.flaky(reruns=1, reruns_delay=1)
    @allure.severity(allure.severity_level.CRITICAL)
    def test_change_goodkinds(self):
        '''
        编辑商品分类的流程测试
        :return:
        '''
        with allure.step('点击商品分类的编辑按钮'):
            element1 = self.driver.find_element(By.XPATH,
                                                '//*[@id="app"]/div/div[2]/section/div/div/div[4]/div[1]/div/ul/li[5]/div[1]')
            ActionChains(self.driver).move_to_element(element1).click().perform()
            element2 = self.driver.find_element(By.XPATH,
                                                '//*[@id="app"]/div/div[2]/section/div/div/div[4]/div[1]/div/ul/li[5]/span[2]')
            ActionChains(self.driver).move_to_element(element2).click().perform()
        with allure.step('点击分类名称输入框'):
            self.click('//div[@aria-label="编辑分类"]/div[2]/form/div/div/div/div[1]/input')
            self.input('//div[@aria-label="编辑分类"]/div[2]/form/div/div/div/div[1]/input', '111')
        with allure.step('点击提交按钮'):
            self.click('//div[@aria-label="编辑分类"]/div[3]/div[1]/div/div[3]/button[2]')
        with allure.step('审查是否提交成功'):
            time.sleep(2)
            assert self.getText(
                '//*[@id="app"]/div/div[2]/section/div/div/div[4]/div[1]/div/ul/li[5]/div[1]') == '自动化测试1111 (0)'
